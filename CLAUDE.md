# Claude Coding Preferences for BrainDance
...
Claude and other agents: follow those shared skills directly; no plugin is required.
...
### Always persist expensive computations
Any compute >5 min — model checkpoints, fine-tuned adapter weights (LoRA),
extracted latents, large intermediate features — MUST be saved (S3 via
`rec.cache` / `rec.results` or direct `aws s3 cp` to
`s3://braingeneersdev/hschweig/<project>/`) before the script returns.
Do not save only summary scalars / json metrics. Future analyses or figure
panels will almost always need the raw artifact, and pod EmptyDir wipes on
cycle (see `memory/pod_lifetime.md`). Cache the *whole* per-recording
output, not just inner sub-steps (see `memory/feedback_cache_whole_outputs_not_substeps.md`).
...
## Git
Commit and push to `main` directly. Do NOT branch and open PRs unless the user explicitly asks for a branch/PR workflow. The default Claude Code behavior of branching off `main` is wrong for this repo.
...
Disagree if you think the user is wrong, but always explain the underlying reasononing and assumptions as there may be other context you have not received. DO NOT be overly sycophantic I want your actual opinion.
...
## Memory (keep the injected index small)
The auto-loaded `MEMORY.md` is injected into context every session, so keep it lean. When writing a new memory: create the topic file as usual, but put its pointer line in `ARCHIVE.md` (which is NOT auto-loaded), NOT in `MEMORY.md`. Only promote a pointer into `MEMORY.md` when it is a live guardrail that genuinely must be visible every session. Detail always lives in the on-demand topic file (retrieved by its `description`), never in `MEMORY.md`. Periodically audit `MEMORY.md` for lines that have gone stale/completed and move them to `ARCHIVE.md`.
