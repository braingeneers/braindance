...
from braindance.core.trainer import TetanusTrainer, generate_tetanus_pattern

import numpy as np
import datetime

params = maxwell_params
params['save_dir'] = './fast' # Path to the data directory, will be created if it doesn't exist
params['name'] = 'cartpole_fast' # Name of the experiment
...
params['config'] = 'config_st.cfg'#'config.cfg' # Path to the config file

params['observation_type'] = 'raw'


# params['stim_electrodes'] = [10825, 12134, 13668, 23819,  16079, 12151, 15859]
# motor_neurons = [7, 790] #643
# sensory_neurons = [0,1] # Indexes of the stim_electrodes array6
# training_neurons= [2,3,4,5,6] # Indexes of the stim neurons

params['stim_electrodes'] = [17414, 16079, 14549, 15430, 11255]# s0, s1, t0, t1 (t0 is near m0)
motor_neurons = [772, 514] #643
sensory_neurons = [0,1] # Indexes of the stim_electrodes array6
training_neurons= [2,3,4] # Indexes of the stim neurons
...
n_signals = 20
training_signals = [generate_tetanus_pattern(training_neurons, stim_count=2, delay_ms=10) for i in range(n_signals)]
# training_signals = [None for i in range(n_signals)]
# freqs = np.linspace(20,40,20)
freqs = 10



# Make trainer
tt = TetanusTrainer(training_signals,freqs, learning_rate=.3)

plug = SmartPlug('coleman', verbose=True)

class Experimenter:
    def __init__(self, params, neuron_list, sensory_neurons, motor_neurons, 
                 training_neurons, plug, n_episodes, trainer=None):
        self.params = params
    ...
    def run_experiment(self):
        ...
        cartpole = CartPolePhase(env, sensory_neurons=self.sensory_neurons, motor_neurons=self.motor_neurons,
                                ...
                                read_period_ms = 150, train_period_ms = 200,
                                wait_period_ms=1000,
                                n_episodes=n_episodes, trainer=tt, artifact_removal=True,
                                continuous=True, assistive=0)
...
if __name__ == '__main__':
    ...
    experimenter = Experimenter(params, neuron_list, sensory_neurons, motor_neurons, training_neurons,
                     plug, n_episodes=n_episodes, trainer=tt)
