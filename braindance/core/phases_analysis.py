...
class HeatmapPhase(AnalysisPhase):
    def __init__(self, name, verbose=False, make_plots=True, make_gif=False, save_plots=False):
        super().__init__()
        
        self.predicted_time = 120 # Hmm?
    ...
    def run(self, analysis=None, filename=None):
        import pandas as pd
        ...
        if analysis is None:
            if filename is None:
                raise ValueError("Must provide either analysis or filename")
            
            analysis = data_loader.AnalysisDAO.from_file_path(filename)


        try:
            self.filename = analysis.file_path
        except:
            self.filename = analysis.params['save_dir'] + '/' + analysis.params['name']
        ...
        try:
            spikes_df = analysis.spikes 
            # spikes_df = data_loader.load_data_maxwell(self.filename, spikes=True)
        except:
            analysis.set_spikes(data_loader.load_data_maxwell(self.filename, spikes=True))
            spikes_df = analysis.spikes
        ...
        if self.make_plots:
            ...
            if self.save_plots:
                plt.savefig(f"{self.filename}_heatmap.png")
            else:
                plt.show()
        ...
        if self.make_plots:
            ...
            if self.save_plots:
                plt.savefig(f"{self.filename}_heatmap2.png")
            else:
        ...
        analysis.select_electrodes(active_electrodes)
        return analysis
    ...
    def info(self):
        info = {}
        info['name'] = self.name
        info['description'] = "Sum up activity on each channel, and save heatmap"
        return info
...
class FootprintPhase(AnalysisPhase):
    ...
    def __init__(self, wind=100, rms_mult=7, spikes_per_channel = 300,
                ...
                load_whole_recording = False, dao = None, verbose=False):
        """
    ...
    def run(self, analysis):
        # Load spikes from recording

        if self.verbose:
            print("Loading spikes")
        all_spikes = analysis.spikes
        ...
        if self.remove_bad:
            analysis.select_channels([c for c in selected_channels if c not in bad_chans])
        ...
        if self.remove_redundant:
            ...
            analysis.select_channels([c for c in selected_channels if c not in to_remove])
...
class CausalAnalysis(AnalysisPhase):
    ...
    def run(self, analysis, stim_electrode_ind = None):
        # Load spikes from recording
        ...
        if self.file_path is None:
            ...
            analysis.set_file_path(file_path)

        mapping = analysis.mapping
        fs_ms = 20

        # First we load the stim log
        if self.verbose:
            print("Loading stim log")

        if analysis.stim_log is None:
            if not (self.stim_log_path is None):
                stim_log = data_loader.adjust_stim_times2(file_path, stim_log_path = self.stim_log_path, 
                                                        stim_offset_ms = 10, 
                                                        tag=self.tag)
            else:
                stim_log = data_loader.adjust_stim_times2(file_path, stim_offset_ms = 10,
                                                        tag=self.tag)
                
            stim_log['stim_electrodes'] = stim_log['stim_electrodes'].apply(lambda x: x[0] if isinstance(x, list) and len(x) > 0 else None)
        else:
            stim_log = analysis.stim_log
            
        # Stim electrodes are a list, so get the first object in each row, we assume there is only one stim electrode per stim
        stim_electrodes = stim_log.stim_electrodes.unique() #list(set(stim_log['stim_electrodes']))
        print("Stim electrodes:", stim_electrodes)
        if stim_electrode_ind is not None:
            stim_electrodes = [stim_electrodes[stim_electrode_ind]]

        n_stims = [(stim_log['stim_electrodes']==s).sum() for s in stim_electrodes]

        if self.channels_of_interest is None:
            self.channels_of_interest = analysis.get_channels(stim_electrodes)
        elif isinstance(self.channels_of_interest, str) and self.channels_of_interest == 'all':
            info = data_loader.load_info_maxwell(file_path)
            self.channels_of_interest = np.arange(info['shape'][0])

        # If we have a clean data path to save to, we will create a new array
        # to store the cleaned data
        
        if self.clean_data_path:
            # n_stims, stim_electrodes, channels, wind_ms
            clean_data = np.zeros((max(n_stims), len(stim_electrodes), len(self.channels_of_interest), self.wind_ms*20))            

        # For each stim electrode, we will loop through all stim times
        # get the window after the stim time, remove artifacts, and threshold
        # to get the spikes and latencies.

        #reactive_spikes is size (n_stim_electrodes, n_channels, self.wind_ms)
        reactive_spikes = np.zeros((len(stim_electrodes), len(self.channels_of_interest), self.wind_ms))
        
        reactive_spikes_times = np.zeros((len(stim_electrodes), len(self.channels_of_interest)), dtype=object)

        ar = ArtifactRemoval(N=self.art_rem_N, nc_start=self.art_rem_N, min_val=-200, max_val=200)
        

        if self.verbose:
            print("Processing Data")

        for stim_electrode_ind, stim_electrode in tqdm(enumerate(stim_electrodes)):
            if self.verbose:
                print(f"Processing electrode {stim_electrode} ({stim_electrode_ind+1}/{len(stim_electrodes)})")
            # ar = ArtifactRemoval(20,20)

            # Get all stim times for this electrode
            stim_times = stim_log['time_mod'][stim_log['stim_electrodes']==stim_electrode]

            # Get the window size in frames
            wind = int(self.wind_ms * 20)

            stim_times = stim_times.to_numpy()*20000
            stim_times = stim_times.astype(int)
            # Load the raw traces around the frames where channel_oi spikes, we need extra time after
            # For how the artifact removal works

            all_windows = data_loader.load_windows_maxwell(file_path, stim_times, window_sz=self.remove_start_frames + wind + self.art_rem_N,
                                                            channels = self.channels_of_interest)


            # Zero first 2ms
            all_windows[:,:,:self.remove_start_frames] = 0

            for ch in tqdm(range(all_windows.shape[1]), leave=False):
                ch_peaks = []
                ch_peaks2 = []
                # fig, ax = plt.subplots(1,1)

                stds = []
                # For each stim repetition, remove artifacts
                for n_rep in range(all_windows.shape[0]):
                    data_i = all_windows[n_rep,ch,self.remove_start_frames:]
                    # Artifact Removal
                    all_windows[n_rep,ch,self.remove_start_frames:], _ = cubic_fit5(data_i,
                                                                                    self.art_rem_N, self.art_rem_N)
                    # ar = ArtifactRemoval(N=self.art_rem_N, nc_start=self.art_rem_N, min_val=-200, max_val=200)
                    # all_windows[n_rep,ch,self.remove_start_frames:] = ar.run(data_i) # Just clean data


                    #Faster
                    # for i in range(data_i.shape[0]-self.art_rem_N):
                    #     clean_data_step, art_step, spike = ar.fit_step(data_i[i+self.art_rem_N])
                    #     if i >= self.art_rem_N:
                    #         data_i[i] = clean_data_step

                    
                    data_i = all_windows[n_rep,ch,self.remove_start_frames:]
                    data_i[-self.art_rem_N:] = 0

                    # if min or max is too high, remove
                    if np.max(data_i) > 200 or np.min(data_i) < -200:
                        #zero out the data
                        all_windows[n_rep,ch,self.remove_start_frames:] = 0
                        
                    # Store the clean data
                    if self.clean_data_path:
                        clean_data[n_rep,stim_electrode_ind,ch,:] = all_windows[n_rep,ch,:self.wind_ms*20]

                    stds.append(np.std(data_i))

                # Calculate median of the stds
                cur_std = np.median(stds)

                # Now find peaks
                for n_rep in range(all_windows.shape[0]):
                    data_i = all_windows[n_rep,ch,self.remove_start_frames:]
                    # Find peaks, we will go sligthly shorter since artifact removal is weird at the end...
                    peaks, _ = find_peaks(-data_i[:-self.art_rem_N], height=cur_std*5, distance=3*fs_ms)
                    
                    # t = np.arange(len(data_i)) / 20
                    # ax.plot(t, data_i)
                    # ax.plot(t[peaks], data_i[peaks], "x")
                    #
                    
                    ch_peaks.extend(peaks)
                    ch_peaks2.append(peaks)
                #Plot the threshold
                # ax.plot(t, -np.ones(len(t))*cur_std*4, '--')
                # ax.plot(t, -np.ones(len(t))*cur_std*11, '--')
                # plt.title(f"Stim Electrode {stim_electrode}, Channel {ch}")
                # plt.show()

                # bin at 1ms and put in reactive_spikes
                ch_reactions = np.asarray(ch_peaks)
                ch_reactions = ch_reactions // 20
                ch_reactions = ch_reactions.astype(int)
                ch_reactions = np.bincount(ch_reactions, minlength=self.wind_ms)
                reactive_spikes[stim_electrode_ind, ch, :] = ch_reactions
                reactive_spikes_times[stim_electrode_ind, ch] = ch_peaks2

        analysis.set_stim_electrodes(stim_electrodes)
        analysis.set_channels_of_interest(self.channels_of_interest)
        analysis.set_reactivity(reactive_spikes)
        analysis.set_reactivity_times(reactive_spikes_times)
        analysis.set_stim_log(stim_log)

        if self.clean_data_path:
            analysis.set_clean_data(clean_data)
        return analysis