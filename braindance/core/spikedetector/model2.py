...
class ModelSpikeSorter(nn.Module):
    ...
    @staticmethod
    def load(detection_model_path, device=None, dtype=None):
        """
        ...
        Args:
            ...
            device: Optional device override; CPU defaults to float32.
            dtype: Optional precision override.
        ...
        if device is not None:
            init_dict["device"] = str(device)
        if dtype is None and torch.device(init_dict.get("device", "cuda")).type == "cpu":
            dtype = torch.float32
        if dtype is not None:
            init_dict["dtype"] = dtype
        model = ModelSpikeSorter(**init_dict)
        model.load_state_dict(torch.load(
            detection_model_path / 'state_dict.pt',
