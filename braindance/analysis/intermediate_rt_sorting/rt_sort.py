...
class RTSort:
    ...
    def sort_offline(self, recording,
        ...
        Args:
            ...
            model_outputs (numpy.ndarray, str, or pathlib.Path, optional):
                ...
                If using model_outputs, argument `recording` should be model_traces
        ...
        intermediate_data = {}
        last_start_frame = list(all_start_frames)[-1]
        num_chars = len(str(last_start_frame))
        ...
        for start_frame in all_start_frames:
            ...
            if isinstance(detections, list):
                detections = {
                    "stringent_crosses": [],
                    "stringent_crosses_not_noise": [],
                    "seq_spike_data": [],
                    "detections": []
                }
            intermediate_data[f"new_chunk_frame_{(num_chars-len(str(start_frame)))*'0'}{start_frame}"] = detections
            continue
    ...
    def sort_chunk(self, chunk, torch_window=None,
        ...
        # For saving intermediate data (e.g. testing on FPGA)
        intermediate_data = {}
        ...
        intermediate_data["stringent_crosses"] = torch.nonzero(peaks & crosses).cpu()
        intermediate_data["stringent_crosses"][:, 0] = torch.tensor(self.seqs_root_elecs)[intermediate_data["stringent_crosses"][:, 0]]
        intermediate_data["stringent_crosses"][:, 1] += self.seq_n_before + self.front_buffer - self.input_size//2  # Relative to start of recording
        # Save non-noise peaks
        intermediate_data["stringent_crosses_not_noise"] = intermediate_data["stringent_crosses"][
            torch.sum(torch_window[:, peak_ind_flat+self.seq_n_before] >= self.loose_thresh_logit, dim=0).cpu() <= self.min_elecs_for_array_noise
        ] 
        ...
        elec_probs, latencies = self.max_pool(spike_window)  # Shape (num_spikes, num_all_comp_elecs, 1)
        ...
        # start = perf_counter()
        latencies_float = latencies.transpose(1, 2).to(self.dtype)  # Shape (num_spikes, 1, num_all_comp_elecs)
        ...
        spike_scores *= can_spike  # Spike scores range from 2.1 to 0.1. If 0, then failed one of the tests. Higher score is better match
        # spike_scores have shape (num_stringent_crossings, num_sequences)

        # Only save latencies and amplitudes for sequences that had a stringent root crossing and save non-zero spike scores
        seq_spike_data = []
        for spike_idx, seq_idx in torch.nonzero(strict_crosses_root):
            this_spike_time = peak_ind_flat[spike_idx] + self.seq_n_before + self.front_buffer - self.input_size//2
            seq_latency_elecs = torch.nonzero(self.seqs_latency_weights[seq_idx]).flatten()
            seq_spike_latencies = latencies_float[spike_idx, 0, seq_latency_elecs] - self.seq_n_before  # Make relative to root
            
            seq_amp_elecs = torch.nonzero(self.seqs_amp_weights[seq_idx]).flatten()
            seq_spike_amps = amps[spike_idx, 0, seq_amp_elecs]

            score = spike_scores[spike_idx, seq_idx]
            
            seq_spike_data.append({
                "seq_id": seq_idx.item(),
                "spike_time": this_spike_time.item(),
                "overlap_score": score.item(),
                "latencies": seq_spike_latencies.cpu(),
                "amps": seq_spike_amps.cpu(),
            })
        intermediate_data["seq_spike_data"] = seq_spike_data
        ...
        intermediate_data['detections'] = [(seq_idx+1, round(spike_time*self.samp_freq) - spike_times_frame_offset - self.input_size//2) for seq_idx, spike_time in detections]
        return intermediate_data   
    ...
    def to(self, device="cpu"):
        # Convert RTSort object to new device (like .to for pytorch tensors)
        self.device = device
        for attr_name in dir(self):
            ...
            if hasattr(attr_value, "to"): 
                if hasattr(attr_value, "device"):
                    attr_value = attr_value.to(device=device)
                # if hasattr(attr_value, "dtype"):  # Need a conditional that if a tensor is Long, it stays Long (e.g. for indexing)
                #     attr_value = attr_value.to(dtype=dtype)
...
def run_detection_model(recording,
    ...
    if compile_model:
        ...
        model_compiled = model.model.conv
...
def setup_cluster(cluster, params, n_cocs=None):
    ...
    # # Test
    # test = np.median(np.abs(traces_windows[:, :, :pre_median_frames]), axis=2)  # (num_crossings, num_max_amp_elecs)
    # test = np.clip(test/0.6745, a_min=0.5, a_max=None)
    
    # pre_medians = np.median(np.abs(traces_windows[:, :, :pre_median_frames]), axis=2)  # (num_crossings, num_max_amp_elecs)
    # pre_medians = np.clip(pre_medians/0.6745, a_min=0.5, a_max=None)
    
    # if not np.all(pre_medians == test):
    #     assert False
...
def main():
    ...
    # rec_path = Path("/data/MEAprojects/primary_mouse/patch_ground_truth/200724/2602/MEA_rec_patch_cell2.raw.h5")
    # inter_path = Path("/data/MEAprojects/primary_mouse/patch_ground_truth/200724/2602/rt_sort_full")

    
    # rt_sort = detect_sequences(rec_path, inter_path, None, 5,
    #                            verbose=True, debug=True)
